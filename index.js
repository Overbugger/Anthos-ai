...
if (!GCP_PROJECT_ID || !GCP_LOCATION || !VERTEX_AI_MODEL_NAME) {
    console.error("FATAL ERROR: Missing required environment variables (GCP_PROJECT_ID, GCP_LOCATION, VERTEX_AI_MODEL_NAME). Please check your .env file.");
...
async function getAiResponse(prompt) {
    ...
    try {
        ...
        if (!aggregatedResponse.candidates?.[0]?.content?.parts?.[0]?.text) {
             console.error("Vertex AI response structure invalid:", JSON.stringify(aggregatedResponse));
            throw new Error('Received an empty or invalid response structure from Vertex AI.');
        }
...
app.post('/api/chat/query', async (req, res, next) => {
    try {
        ...
        console.log(`Received query: "${question}" with ${transactions.length} transactions.`);
...
Be concise and directly answer the question. Today's date is ${new Date().toLocaleDateString('en-CA')}.
...
app.listen(PORT, () => {
    ...
    console.log(`API Endpoint available at: http://localhost:${PORT}/api/chat/query (POST)`);
    console.log(`Health Check available at: http://localhost:${PORT}/health (GET)`);
    console.log(`---------------------------------------------------------`);
});